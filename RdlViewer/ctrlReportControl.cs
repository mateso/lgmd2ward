...
namespace RdlViewer
{
    public partial class ctrlReportControl : UserControl
    {
        public ctrlReportControl(string strAppConnection,string strReportName, string [] strSelectionOption)
        {
            InitializeComponent();
            this.reportViewer1.Drillthrough += new DrillthroughEventHandler(DrillthroughEventHandler);
            this.connectionProperty = strAppConnection;
            this.ReportName = strReportName;
            openRDL(strSelectionOption);
        }
        public ctrlReportControl(string strAppConnection, string strReportName)
        {
            InitializeComponent();
            this.reportViewer1.Drillthrough += new DrillthroughEventHandler(DrillthroughEventHandler);
            this.connectionProperty = strAppConnection;
            ...
            ReportParameterInfoCollection reportParams, ReportDataSourceCollection dataSources)
        {
            ...
            try
            {
                ...
                {
                    DataTable dataTable = reportInfo.GetData(dataSetName, reportParams, connectionProperty);
                    if (dataTable == null)
                        return;
        ...
        private string connectionProperty { get; set; }
        private string ReportName { get; set; }
        public bool ShowDialogue { get; set; }
        private void LoadReport(string rdlFilename, string[] strSelectionOption)
        {
            ...
            {
                using (ParameterDialog dialog = new ParameterDialog(strSelectionOption))
                {
                    dialog.ReportViewer = this.reportViewer1;
                    if (ShowDialogue == false)//(strSelectionOption.Length == 1)
                    {
                        dialog.SelectedValues = strSelectionOption;
                        dialog.ParameterDialogLoad(strSelectionOption);
                    }
        ...
        private void LoadDataIntoReport(Microsoft.Reporting.WinForms.LocalReport report)
        {
        ...
        void DrillthroughEventHandler(object sender, Microsoft.Reporting.WinForms.DrillthroughEventArgs e)
        {
        ...
        private void openRDLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    this.Text = Path.GetFileName(openFileDialog1.FileName) + " - RDL Viewer";
                }
                catch (Exception ex)
                {
        ...
        private void openRDL(string[] strSelectionOption)
        {
            //openFileDialog1.FileName = "";
            //openFileDialog1.Filter = "RDL Files (*.rdl)|*.rdl|All Files (*.*)|*.*";
            //if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    LoadReport(this.ReportName, strSelectionOption);
                    this.Text = Path.GetFileName(this.ReportName) + " - LGMD2";
                }
                catch (Exception ex)
                {
                    string message = GetExceptionMessage(ex);
        ...
        private void openRDL()
        {
            ...
            //if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    this.Text = Path.GetFileName(this.ReportName) + " - RDL Viewer";
                }
                catch (Exception ex)
                {
                    string message = GetExceptionMessage(ex);
